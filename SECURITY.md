...
**The root password is the only thing in front of SSH.** The notebook generates a strong one and prints it once. Do not replace it with something guessable, and do not paste it into a shared notebook output — Colab's `private_outputs` setting keeps outputs out of the saved `.ipynb`, but a screenshot or a shared screen defeats that.
